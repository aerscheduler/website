...
export const SCREENSHOTS: ScreenshotSpec[] = [
  ...
  },
  {
    ...
    dataState:
      "Owner, on the not-yet-connected page: the subtitle and the Connect QuickBooks card (three short lines, starting 'Connecting only reads your company. Nothing is sent to QuickBooks yet.', then the Connect QuickBooks button), which is the step the article puts this image under. Recaptured 2026-09-20 from the test org after the redesign. The CONNECTED version of this screen (the left list of sections: Overview, Needs attention, Activity, Settings, Connection) is deliberately NOT what this shows, and cannot be staged: connecting needs a real Intuit company and a real OAuth sign-in, and a hand-written settings row leaves the page reading its item and account lists live from Intuit, i.e. a picture of a broken integration. Capture the connected state only from an org that is genuinely connected.",
