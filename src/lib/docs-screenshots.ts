...
export const SCREENSHOTS: ScreenshotSpec[] = [
  ...
  },
  {
    ...
    dataState:
      "Owner, on the not-yet-connected page: the subtitle and the Connect QuickBooks card (three short lines, starting 'Connecting only reads your company. Nothing is sent to QuickBooks yet.', then the Connect QuickBooks button), which is the step the article puts this image under. Once connected, the page becomes a left list of sections (Overview, Needs attention, Activity, Settings, Connection) with setup as a one-question-at-a-time dialog. Recapture from the production test org after the 2026-09-18 QuickBooks redesign deploys; the file on disk still shows the older three-step strip. The CONNECTED version of this screen cannot be staged locally and the file on disk is deliberately the disconnected one. Connecting needs a real Intuit sandbox company and a real OAuth sign-in, and a hand-written settings row does not stand in: the Configuration section loads Products and Services live from Intuit, so a fabricated token leaves the income item reading 'No active items in QuickBooks' next to a Sync toggle that is on, which is a picture of a broken integration rather than a working one. Capture the connected state only from an org that is genuinely connected.",
